...
Funkcia prijíma ako parameter ukazovateľ na štruktúru URL_SLICED, ktorá obsahuje informácie o protokole, domene,
porte a ceste k súboru na vzdialenom servere.

Funkcia vytvorí socket pomocou funkcie socket() a následne sa pokúsi vytvoriť konektivitu
s vzdialeným serverom pomocou funkcie connect().
...
int serverConnection(URL_SLICED *slicedURL)
{
  ...
  printf("Connecting to %s [%s]... Port:%d\n", slicedURL->domain, ipaddr, atoi(slicedURL->port));
